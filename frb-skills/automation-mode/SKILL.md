...
Run automation under a bounded timeout, chain build and launch with `&&`, and assert the requested artifact exists. Responses use stdout; keep diagnostics on stderr or `Debug.WriteLine` so NDJSON remains parseable.
...
- **`Engine.Gum.Cursor` is null while automation is active.** That property casts Gum's cursor to Gum's concrete type, and automation has installed its own; read `Gum.Forms.FormsUtilities.Cursor` instead. Two injected presses within `ICursor.DoubleClickThreshold` of stepped time register as a double press and double click exactly as a real mouse would (a `TextBox` selects all its text on the second push).
